...
bool HTU21DF::getHumidityComp(float *humd, bool forceUpdate) {
    float h, t;

    if (!getHumidity(&h, forceUpdate))
        return false;

    if (!getTemperature(&t, forceUpdate))
        return false;
...
bool HTU21DF::getDewPoint(float *dewPoint, bool forceUpdate) {
    float h, t, paritalPressure;
